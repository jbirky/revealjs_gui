...
export function generateRevealHTML(presentation) {
  ...
  presentation.slides.forEach((slide, slideIndex) => {
    ...
    const elementsHtml = (slide.elements || [])
      ...
      .map(el => {
        ...
        if (el.type === 'html') {
          ...
          return `<iframe${dataId}${fragClass}${fragIdx}${gsapAttrs} src="${dataUrl}" style="${style}border:none;background:transparent;" scrolling="no"></iframe>`
        }
        if (el.type === 'p5') {
        ...
        if (el.type === 'timeline') {
          ...
          return `<div${dataId}${fragClass}${fragIdx}${gsapAttrs} style="${style}position:relative;">${svg}${expandData}</div>`
        }
        if (el.type === 'chart') {
    ...
    slideSectionHtmlByIndex.set(slideIndex, `    <section${bgAttrs}${autoAnimateAttr}${autoAnimateDurAttr}${autoAnimateEasingAttr}${perSlideTransition}${customTransAttr}${perSlideSpeed} style="padding:0;width:${slideW}px;height:${slideH}px;overflow:hidden;font-size:16px;">\n${elementsHtml}\n${footerHtml}\n${gridHtml}\n${sideCitationsHtml}\n      ${notes}\n    </section>`)
  })
...
<head>
  ...
  <style>
    ...
    /* Heading sizes matching browser defaults (TipTap editor uses these) */
    .reveal .slides section h1 { font-size: 2em; font-weight: bold; line-height: 1.2; margin: 0; }
    .reveal .slides section h2 { font-size: 1.5em; font-weight: bold; line-height: 1.2; margin: 0; }
    .reveal .slides section h3 { font-size: 1.17em; font-weight: bold; line-height: 1.2; margin: 0; }
    .reveal .slides section h4 { font-size: 1em;   font-weight: bold; line-height: 1.2; margin: 0; }
    .reveal .slides section p  { margin: 0; }
    .reveal .slides section ul,
    .reveal .slides section ol { padding-left: 24px; margin: 0; }
    .reveal .slides section li { margin-bottom: 0; }
...
function generatePrintHTML(presentation) {
  ...
  const pagesHtml = pages.map(({ slide, maxIdx }, pageIndex) => {
    ...
    return `<div class="slide-page" style="${bgStyle}font-size:16px;">\n${elementsHtml}\n${footerHtml}\n</div>`
  }).join('\n')
